...
import { CalendarDays, ChevronLeft, ChevronRight, ChevronDown, Folder, LayoutDashboard, ListTodo, Menu, Plus, Search, Settings, Trash2, Users, X, Download, Upload, Save, KanbanSquare, AlertTriangle, Archive, MapPin, Link2, RotateCcw, Banknote, Bell } from 'lucide-react';
...
function emptyProjectForm(): Project { return { id: '', name: '', color: '#38bdf8', status: 'Active', startMonth: new Date().toISOString().slice(0, 7), requester: '', pic: '', outputLandscape: '', outputVertical: '', distribusi: '', lokasi1: '', lokasi2: '', folderLink: '', budget: '', flag: '', thumbnail: '', notes: '', archived: false }; }
...
export default function Home() {
  ...
  function switchPage(label: Page) { setPage(label); if (label === 'Kalender') setView('Bulan'); if (label === 'Timeline') setView('Timeline'); if (label === 'Projects' || label === 'PIC / Team' || label === 'Pengaturan') setDrawerOpen(false); }
  ...
  function saveProject() {
    ...
    const p = { ...projectForm, id, budget: parseRupiahInput(String(projectForm.budget ?? '')) } as Project;
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={onClose}>
    <div className="max-h-[92vh] w-full max-w-2xl overflow-auto rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="grid gap-x-4 md:grid-cols-2">
        ...
        <Field label="Budget (Rp)"><input inputMode="numeric" className="input" value={Number(form.budget) ? Number(form.budget).toLocaleString('id-ID') : ''} onChange={(e) => setForm({ ...form, budget: parseRupiahInput(e.target.value) })} placeholder="0"/></Field>

...
function picList(pic?: string): string[] { return String(pic || '').split(',').map((s) => s.trim()).filter(Boolean); }
function picMatch(pic: string | undefined, name: string): boolean { return picList(pic).includes(name); }
...
export default function Home() {
  ...
  const filteredTasks = useMemo(() => tasks.filter((task) => {
    ...
    return matchesSearch && pStatusOk && (filter.projectId === 'all' || task.projectId === filter.projectId) && (filter.pic === 'all' || picMatch(task.pic, filter.pic)) && (filter.status === 'all' || task.status === filter.status) && (filter.category === 'all' || task.category === filter.category);
  ...
  const buildAiContext = () => {
    ...
    pics.forEach((pic) => { const c = tasks.filter((t) => picMatch(t.pic, pic.name) && effectiveStatus(t) !== 'Canceled' && effectiveStatus(t) !== 'Done').length; lines.push(`- ${pic.name} | ${pic.role || '-'} | ${c}`); });
    return lines.join('\n');
  };
  ...
  function switchPage(label: Page) { setPage(label); setSidebarOpen(false); if (label === 'Kalender') setView('Bulan'); if (label === 'Timeline') setView('Timeline'); if (label === 'Projects' || label === 'PIC / Team' || label === 'Pengaturan' || label === 'Dashboard') setDrawerOpen(false); }
  ...
  function renamePic(from: string, to: string) {
    ...
    setTasks((cur) => cur.map((t) => picList(t.pic).includes(from) ? { ...t, pic: picList(t.pic).map((n) => n === from ? to : n).join(', ') } : t));
  ...
  return <main className="min-h-screen bg-slate-50">
    {sidebarOpen && <div onClick={() => setSidebarOpen(false)} className="fixed inset-0 z-40 bg-slate-950/50 backdrop-blur-sm lg:hidden" aria-hidden />}
    <aside className={`fixed left-0 top-0 z-50 flex h-[100dvh] w-72 max-w-[82%] flex-col bg-slate-950 text-white shadow-2xl transition-transform duration-300 ease-out lg:z-20 lg:max-w-none lg:shadow-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
      <div className="flex items-center gap-3 px-6 py-6"><CalendarDays size={34}/><div className="min-w-0 flex-1"><h1 className="truncate text-xl font-bold">{settings.workspaceName}</h1><p className="text-sm text-slate-300">Multimedia Team</p></div><button onClick={() => setSidebarOpen(false)} className="rounded-lg p-1.5 text-slate-300 hover:bg-white/10 lg:hidden" aria-label="Tutup menu"><X size={20}/></button></div>
...
function Header({ page, onMenu, filter, setFilter, exportJson, currentUser, role, logout, notifItems, unreadCount, onMarkSeen, onPickTask }: any) {
  ...
  return <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b bg-white/90 px-5 backdrop-blur lg:px-8">
    <div className="flex items-center gap-5"><button onClick={onMenu} className="rounded-lg p-2 hover:bg-slate-100 lg:hidden" aria-label="Buka menu"><Menu size={20}/></button><div><h2 className="text-lg font-bold sm:text-xl">{page}</h2><p className="hidden text-sm text-slate-500 sm:block">Kelola timeline project tim multimedia</p></div></div>
    ...
    {searchOpen && <div className="absolute inset-x-0 top-full z-30 border-b bg-white p-3 shadow-sm md:hidden"><div className="flex items-center gap-2 rounded-xl border bg-white px-3 py-2"><Search size={18} className="text-slate-400"/><input autoFocus value={filter.search} onChange={(e) => setFilter({ ...filter, search: e.target.value })} placeholder="Cari task, project, PIC..." className="w-full outline-none" /><button onClick={() => setSearchOpen(false)} className="text-slate-400 hover:text-slate-600"><X size={18}/></button></div></div>}
    <div className="flex items-center gap-2 sm:gap-3">
      <button onClick={() => setSearchOpen((v) => !v)} className="rounded-xl border p-2.5 hover:bg-slate-100 md:hidden" aria-label="Cari"><Search size={18}/></button>
      ...
      <button onClick={exportJson} className="hidden rounded-xl border px-4 py-2 text-sm font-semibold hover:bg-slate-100 sm:inline-block">Export</button><button onClick={logout} className="rounded-xl border px-4 py-2 text-sm font-semibold hover:bg-slate-100">Logout</button><div className="grid h-10 w-10 place-items-center rounded-full bg-slate-200 font-bold">{currentUser?.name?.charAt(0) || 'U'}</div><div className="hidden text-sm md:block"><p className="font-semibold">{currentUser?.name}</p><p className="text-xs text-slate-500">{role}</p></div>
    </div>
  </header>;
}
function KpiRow({ kpis }: { kpis: { total: number; progress: number; overdue: number; dueSoon: number } }) { return <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4"><Kpi title="Total Task" value={kpis.total}/><Kpi title="Progress" value={kpis.progress}/><Kpi title="Overdue" value={kpis.overdue} warning/><Kpi title="Deadline ≤ 3 Hari" value={kpis.dueSoon} warning/></div>; }
...
function CalendarToolbar({ month, setMonth, nextMonth, view, setView, openCreate, zoom, setZoom }: any) {
  const years = Array.from({ length: 9 }, (_, i) => 2023 + i);
  return <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:justify-between">
    <div className="flex flex-wrap items-center gap-2">
      <button onClick={() => nextMonth(-1)} className="rounded-xl border bg-white p-3 hover:bg-slate-100"><ChevronLeft size={18}/></button>
      <button onClick={() => nextMonth(1)} className="rounded-xl border bg-white p-3 hover:bg-slate-100"><ChevronRight size={18}/></button>
      <button onClick={() => setMonth(new Date())} className="rounded-xl border bg-white px-4 py-3 font-semibold hover:bg-slate-100">Hari ini</button>
      <select value={month.getMonth()} onChange={e => setMonth(new Date(month.getFullYear(), Number(e.target.value), 1))} className="rounded-xl border bg-white px-3 py-3 text-base font-bold outline-none sm:text-lg">{monthNames.map((m, i) => <option key={m} value={i}>{m}</option>)}</select>
      <select value={month.getFullYear()} onChange={e => setMonth(new Date(Number(e.target.value), month.getMonth(), 1))} className="rounded-xl border bg-white px-3 py-3 text-base font-bold outline-none sm:text-lg">{years.map(y => <option key={y}>{y}</option>)}</select>
    </div>
    <div className="flex flex-wrap items-center gap-2">
      <div className="flex max-w-full overflow-x-auto rounded-xl border bg-white p-1">{viewButtons.map(v => <button key={v} onClick={() => { setView(v); if (v === 'Minggu') setMonth(new Date()); }} className={`shrink-0 rounded-lg px-3 py-2 text-sm font-semibold sm:px-4 ${view === v ? 'bg-blue-50 text-blue-700' : 'text-slate-600'}`}>{v}</button>)}</div>
      <label className="hidden items-center gap-2 rounded-xl border bg-white px-3 py-2 text-sm lg:flex">Zoom <input type="range" min="55" max="120" value={zoom} onChange={(e) => setZoom(Number(e.target.value))}/></label>
      <button onClick={() => openCreate(todayIso())} className="ml-auto flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-semibold text-white shadow-soft lg:ml-0"><Plus size={18}/>Tambah Task</button>
    </div>
  </div>; }
...
function DashboardPage({ tasks, projects }: { tasks: Task[]; projects: Project[] }) {
  ...
  const picRows = Array.from(new Set(periodTasks.flatMap((t) => picList(t.pic)))).map((name) => ({ name, count: periodTasks.filter((t) => picMatch(t.pic, name)).length, done: periodTasks.filter((t) => picMatch(t.pic, name) && effectiveStatus(t) === 'Done').length })).sort((a, b) => b.count - a.count).slice(0, 10);
...
function TeamPage({ pics, setPics, tasks, projects, canManage, onRenamePic }: { pics: Pic[]; setPics: (p: Pic[]) => void; tasks: Task[]; projects: Project[]; canManage: boolean; onRenamePic: (from: string, to: string) => void }) {
  ...
  const countFor = (name: string) => tasks.filter((t) => picMatch(t.pic, name) && activeIds.has(t.projectId)).length;
  const perProject = (name: string) => activeProjects.map((p) => ({ p, count: tasks.filter((t) => picMatch(t.pic, name) && t.projectId === p.id).length })).filter((x) => x.count > 0);
...
function SettingsPage({ settings, setSettings, categories, setCategories, statuses, setStatuses, requesters, setRequesters, users, onSaveUser, onRemoveUser, canManageUsers }: any) { const [email, setEmail] = useState(''); const [name, setName] = useState(''); const [password, setPassword] = useState(''); const [role, setRole] = useState('Member'); const addUser = () => { if (!canManageUsers || !email.trim() || !password.trim()) return; onSaveUser({ id: '', email: email.trim().toLowerCase(), name: name.trim() || email.trim(), role, active: true, team: 'Multimedia' }, password.trim()); setEmail(''); setName(''); setPassword(''); }; return <div className="grid gap-4 xl:grid-cols-2"><WorkspaceCard settings={settings} setSettings={setSettings}/><div className="rounded-2xl border bg-white p-4 shadow-soft"><h3 className="mb-4 text-lg font-bold">Status & Kategori</h3><ListEditor title="Status" items={statuses} setItems={setStatuses}/><ListEditor title="Kategori" items={categories} setItems={setCategories}/><ListEditor title="Requester" items={requesters} setItems={setRequesters}/><p className="text-xs text-slate-400">PIC dikelola lengkap (role, warna, foto) di menu PIC / Team.</p></div><div className="rounded-2xl border bg-white p-4 shadow-soft xl:col-span-2"><h3 className="mb-1 text-lg font-bold">Users & Role Access</h3><p className="mb-4 text-xs text-slate-500">Password disimpan ter-hash di server dan tidak pernah dikirim ke browser. Isi kolom password hanya jika ingin mengganti.</p>{canManageUsers && <div className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_160px_120px]"><input className="input" value={email} onChange={e=>setEmail(e.target.value)} placeholder="email@company.com"/><input className="input" value={name} onChange={e=>setName(e.target.value)} placeholder="Nama"/><input className="input" type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password"/><select className="input" value={role} onChange={e=>setRole(e.target.value)}><option>Admin</option><option>Manager</option><option>Member</option><option>Viewer</option></select><button onClick={addUser} className="rounded-xl bg-blue-600 px-4 font-semibold text-white">Tambah</button></div>}<div className="space-y-2">{users.map((u: AppUser) => <UserRow key={u.id} user={u} canManageUsers={canManageUsers} onSave={onSaveUser} onRemove={onRemoveUser}/>)}</div><p className="mt-3 text-xs text-slate-500">Role: Admin bisa semua, Manager master data tanpa hapus user, Member tambah/edit task, Viewer hanya lihat/export.</p></div></div>; }
function UserRow({ user, canManageUsers, onSave, onRemove }: { user: AppUser; canManageUsers: boolean; onSave: (u: AppUser, pw?: string) => void; onRemove: (id: string) => void }) {
  const [name, setName] = useState(user.name); const [role, setRole] = useState(user.role); const [active, setActive] = useState(user.active); const [pwd, setPwd] = useState('');
  useEffect(() => { setName(user.name); setRole(user.role); setActive(user.active); }, [user]);
  if (!canManageUsers) return <div className="rounded-xl border p-3 text-sm"><div className="flex items-center justify-between gap-2"><span className="truncate font-semibold">{user.name}</span><span className="shrink-0 rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium">{user.role}</span></div><p className="truncate text-slate-500">{user.email}</p><p className="mt-1 text-xs text-slate-400">{user.active ? 'Aktif' : 'Nonaktif'} · Password {user.hasPassword ? 'terset' : 'belum diset'}</p></div>;
  return <div className="rounded-xl border p-3"><div className="mb-2 flex items-center justify-between gap-2"><p className="truncate text-sm font-semibold text-slate-700">{user.email}</p><label className="flex shrink-0 items-center gap-1.5 text-xs font-medium text-slate-500"><input type="checkbox" checked={active} onChange={(e) => setActive(e.target.checked)} />Aktif</label></div><div className="grid gap-2 sm:grid-cols-2"><input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nama" /><select className="input" value={role} onChange={(e) => setRole(e.target.value)}><option>Admin</option><option>Manager</option><option>Member</option><option>Viewer</option></select><input className="input sm:col-span-2" type="password" value={pwd} onChange={(e) => setPwd(e.target.value)} placeholder={user.hasPassword ? 'ubah password' : 'set password'} /></div><div className="mt-2 flex justify-end gap-2"><button onClick={() => { onSave({ ...user, name, role, active }, pwd || undefined); setPwd(''); }} className="rounded-lg bg-blue-600 px-4 py-1.5 text-sm font-semibold text-white">Simpan</button><button onClick={() => onRemove(user.id)} className="rounded-lg border border-red-200 px-4 py-1.5 text-sm font-semibold text-red-600 hover:bg-red-50">Hapus</button></div></div>; }
...
function AssistantPanel({ currentUser, buildContext, tasks, projects, onOpenTask, onOpenProject, rootClass, onClose }: { currentUser: AppUser | null; buildContext: () => string; tasks: Task[]; projects: Project[]; onOpenTask: (t: Task) => void; onOpenProject: (p: Project) => void; rootClass?: string; onClose?: () => void }) {
  ...
  return <div className={rootClass || 'sticky top-20 flex h-[calc(100vh-80px)] flex-col'}>
    <div className="flex items-center justify-between border-b px-4 py-3">
      ...
      <div className="flex items-center gap-1">{messages.length > 0 && <button title="Bersihkan percakapan" onClick={() => { if (window.confirm('Hapus riwayat percakapan asisten?')) setMessages([]); }} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-red-600"><Trash2 size={15}/></button>}{onClose && <button title="Tutup" onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100"><X size={18}/></button>}</div>
    </div>
